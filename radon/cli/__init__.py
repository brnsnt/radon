...
def cc(paths, min=_cfg.get_value('cc_min', str, 'A'),
    ...
    with outstream(output_file) as stream:
        log_result(
            harvester, json=json, xml=xml, codeclimate=codeclimate, stream=stream
        )
...
def cgc(
    ...
    """Analyze the given Python modules and compute Cognitive
    Complexity (CC).
    ...
    harvester = CGCHarvester(paths, config)
    with outstream(output_file) as stream:
        log_result(
            harvester, json=json, xml=xml, codeclimate=codeclimate, stream=stream
        )
